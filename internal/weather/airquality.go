...
// Air-Quality API for one site.
func (p *Provider) fetchAirQuality(ctx context.Context, lat, lon float64) (aqResponse, error) {
	url := fmt.Sprintf("%s?latitude=%s&longitude=%s&hourly=aerosol_optical_depth,dust,pm2_5,pm10&past_days=1&forecast_days=2&timezone=UTC",
		p.airQualityURL, ftoa(lat), ftoa(lon))
	var resp aqResponse
