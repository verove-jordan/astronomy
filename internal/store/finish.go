...
// FinishIteration is one supervised finish render + decision (v2). params/metrics are JSON blobs
// (the tuned composite params and the measured finish metrics).
...
// params/metrics are sent as jsonb. Satisfies pipeline.FinishIterStore.
func (s *Store) CreateFinishIteration(ctx context.Context, jobID int64, iter int, params, metrics []byte, detScore, modelScore, combined float64, reasoning string) (int64, error) {
	if len(params) == 0 {
		params = []byte("{}")
	}
	if len(metrics) == 0 {
	...
	err := s.pool.QueryRow(ctx,
		`INSERT INTO finish_iterations(job_id, iteration, params, metrics, det_score, model_score,
		    combined_score, reasoning, chosen, created_at, updated_at)
		 VALUES($1,$2,$3,$4,$5,$6,$7,$8,false,$9,$9) RETURNING id`,
		jobID, iter, json.RawMessage(params), json.RawMessage(metrics),
...
func (s *Store) ListFinishIterations(ctx context.Context, jobID int64) ([]FinishIteration, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT id, job_id, iteration, params, metrics, det_score, model_score, combined_score,
		    reasoning, chosen, created_at, updated_at
