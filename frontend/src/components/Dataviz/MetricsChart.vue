<script setup lang="ts">
import { computed } from "vue";
import { use } from "echarts/core";
import { BarChart, LineChart } from "echarts/charts";
import {
  GridComponent,
  TooltipComponent,
  LegendComponent,
} from "echarts/components";
import { CanvasRenderer } from "echarts/renderers";
import VChart from "vue-echarts";
import { useI18n } from "vue-i18n";
import type { GradeMetric } from "@/types";
import {
  CHART_AXIS,
  CHART_LINE,
  CHART_REJECTED,
  filterHex,
} from "@/constants/colors";

use([
  BarChart,
  LineChart,
  GridComponent,
  TooltipComponent,
  LegendComponent,
  CanvasRenderer,
]);

const props = defineProps<{ metrics: GradeMetric[]; filter?: string }>();
const { t } = useI18n();
const keptColor = computed(() => filterHex(props.filter));

const option = computed(() => ({
  tooltip: { trigger: "axis" },
  legend: {
    data: [t("fields.fwhm"), t("fields.roundness")],
    textStyle: { color: CHART_AXIS },
  },
  grid: { left: 45, right: 45, top: 30, bottom: 30 },
  xAxis: {
    type: "category",
    data: props.metrics.map((m) => "#" + m.index),
    axisLabel: { color: CHART_AXIS },
  },
  yAxis: [
    { type: "value", name: t("fields.fwhm"), axisLabel: { color: CHART_AXIS } },
    {
      type: "value",
      name: t("fields.roundness"),
      min: 0,
      max: 1,
      axisLabel: { color: CHART_AXIS },
    },
  ],
  series: [
    {
      name: t("fields.fwhm"),
      type: "bar",
      data: props.metrics.map((m) => ({
        value: m.fwhm,
        itemStyle: { color: m.rejected ? CHART_REJECTED : keptColor.value },
      })),
    },
    {
      name: t("fields.roundness"),
      type: "line",
      yAxisIndex: 1,
      data: props.metrics.map((m) => m.roundness),
      lineStyle: { color: CHART_LINE },
      itemStyle: { color: CHART_LINE },
    },
  ],
}));
</script>

<template>
  <VChart :option="option" autoresize class="h-72 w-full" />
</template>
